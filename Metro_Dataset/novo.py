...
failure_report['Start Time'] -= pd.Timedelta(hours=1)
...
for n in range(len(data_filter)):
    if data_filter.iloc[n]['failure'] == 1 and data_filter.iloc[n]['Oil_temperature'] > 86:
        count = count+1
    else:
        certa = certa+1
...
for n in range(len(test_predictions_with_rules)):
    if test_predictions_with_rules[n] == 1 and x_test.iloc[n]['median_oil_temperature'] > 0 and x_test.iloc[n]['median_oil_temperature'] < 67 and x_test.iloc[n]['Oil_level'] == 0:
        #print(x_test.iloc[n])
...
'''
for n in range(len(test_predictions_with_rules)):
    if test_predictions_with_rules[n] == 0 and y_test.iloc[n] == 1:
        oil_temperatures.append(x_test.iloc[n]['median_motor_current'])

if oil_temperatures:
    median_temperature = np.median(oil_temperatures)
    max_temperature = np.max(oil_temperatures)
    min_temperature = np.min(oil_temperatures)
    std_temperature = np.std(oil_temperatures)

    print("Median Oil Temperature:", median_temperature)
    print("Maximum Oil Temperature:", max_temperature)
    print("Minimum Oil Temperature:", min_temperature)
    print("Standard Deviation of Oil Temperature:", std_temperature)
else:
    print("No instances found where prediction is 0 and actual label is 1.")
'''
'''
count=0
certa=0
...
for n in range(len(test_predictions_with_rules)):
    if test_predictions_with_rules[n] == 0 and x_test.iloc[n]['median_motor_current'] > 5.1 and x_test.iloc[n]['median_motor_current'] < 5.3 and x_test.iloc[n]['Oil_level'] == 1 and x_test.iloc[n]['median_h1'] < 0 and x_test.iloc[n]['median_h1'] > -0.007 and x_test.iloc[n]['LPS'] == 1 and x_test.iloc[n]['Pressure_switch'] == 1 and x_test.iloc[n]['Caudal_impulses'] == 1 and x_test.iloc[n]['median_towers'] == 0:
        #print(x_test.iloc[n])
        test_predictions_with_rules[n] = 1
        count = count+1
        if y_test[n] == 1:
...
for n in range(len(test_predictions_with_rules)):
    if test_predictions_with_rules[n] == 0 and y_test.iloc[n]==1:
        if x_test.iloc[n]['median_motor_current'] > 5 and x_test.iloc[n]['median_motor_current'] < 5.4:
            if x<100:
                print(x_test.iloc[n])
                x=x+1
'''
...
for n in range(len(test_predictions_with_rules)):
    if (test_predictions_with_rules[n] == 1 and x_test.iloc[n]['median_oil_temperature'] < 71):
...
test_accuracy = accuracy_score(y_test, test_predictions_with_rules)
...
# Print the evaluation metrics
print("\nTest Accuracy (with expert rules):", test_accuracy)
...
print("\nPrecision Difference:", (test_precision_e-test_precision)*100)
print("\nF1-score Difference:", (test_f1_score_e-test_f1_score)*100)