...
median_tp2 = data.set_index('timestamp').resample('10min')['TP2'].median()

# Merge the calculated median values back to the original DataFrame
data = data.merge(median_tp2, how='left', left_on=data['timestamp'].dt.floor('10min'), right_index=True)
...
# Resample the DataFrame to 10-minute intervals and calculate median of 'Reservoirs'
median_reservoirs = data.set_index('timestamp').resample('10min')['Reservoirs'].median()

# Merge the calculated median values back to the original DataFrame
data = data.merge(median_reservoirs, how='left', left_on=data['timestamp'].dt.floor('10min'), right_index=True)
...
param_grid = {
    ...
    'classifier__learning_rate': [0.1, 0.08, 0.12],
    'classifier__n_estimators': [100, 200],
    'classifier__max_depth': [1],
    'classifier__min_child_samples': [1],
    'classifier__reg_lambda': [0.1, 0.08]
}

'''
param_grid = {
    'learning_rate': [0.01, 0.012, 0.014, 0.008],
    'n_estimators': [390, 400, 420, 450],
    'max_depth': [1, 2],
...
# Create Grid Search
grid_search = GridSearchCV(estimator=pipeline, param_grid=param_grid, cv=5, scoring=scoring, verbose=5)
...
if test_recall > 0 and test_precision > 0:
    print("\nSalvou o Modelo")
    joblib.dump(best_model, 'best_model__PCA_Lightgbm.pkl')
    '''